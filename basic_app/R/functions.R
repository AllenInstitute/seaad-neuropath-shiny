...
build_qnp_grouped_boxplot <- function(long_data, x_field, x_label, facet = FALSE, y_range = NULL) {
  ...
  p <- ggplot2::ggplot(long_data, ggplot2::aes(x = .data[[x_field]], y = value, fill = measure)) +
    ggplot2::geom_boxplot(outlier.shape = NA, position = ggplot2::position_dodge(width = 0.8), alpha = 0.3, width = 0.7) +
    ggplot2::geom_point(
  ...
  if (facet) p <- p + ggplot2::facet_wrap(~region)
  p
}

# scatter: x is numeric (CPS or a QNP measure), one colored, hoverable
# and clickable series per y measure. x_is_cps expands the x-axis to
# always include 0 and 1 (CPS's nominal range), ticked every 0.5 — a
